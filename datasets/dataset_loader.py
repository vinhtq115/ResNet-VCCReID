...
def build_trainloader() -> Tuple[DataLoader, VideoDataset, RandomIdentitySampler]:
    """
    Build Train Loader
    """
...
def build_testloader() -> Tuple[DataLoader, DataLoader, TestDataset, TestDataset]:
    """
    Build query and gallery loader
    """
    if config.TEST.TEST_MODE == 'all':
        query_data_path = osp.join(config.DATA.ROOT, config.TEST.TEST_SET, 'query.pkl')
        gallery_data_path = osp.join(config.DATA.ROOT, config.TEST.TEST_SET, 'gallery.pkl')
    else:
        query_data_path = osp.join(config.DATA.ROOT, config.TEST.TEST_SET, config.TEST.TYPE, f'query_{config.TEST.TEST_MODE}.pkl')
        gallery_data_path = osp.join(config.DATA.ROOT, config.TEST.TEST_SET, config.TEST.TYPE, f'gallery_{config.TEST.TEST_MODE}.pkl')
